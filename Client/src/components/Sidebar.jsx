...
const Sidebar = () => {
  ...
  const navigate = useNavigate();
  const [selectedSubcategoryId, setSelectedSubcategoryId] = useState(null);
  const [isOpen, setIsOpen] = useState(false); // For category accordion
    ...
    <>
      ...
      <div className="md:hidden fixed top-5 left-5 z-50">
        <button
          ...
          className="text-2xl text-primary bg-white p-2 rounded shadow"
        >
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-screen w-64 bg-white p-4 pt-20 font-poppins overflow-y-auto shadow transition-transform z-40
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 md:w-1/5`}
      >
        <h2 className="text-lg font-semibold mb-4 text-primary">Categories</h2>
        <button
          className="mb-2 underline hover:underline hover:text-primary"
